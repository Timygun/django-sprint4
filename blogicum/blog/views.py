...
def profile(request, username):
    ...
    if request.user.is_authenticated and request.user == author:
        qs = Post.objects.filter(author=author).select_related('author', 'category', 'location')
    else:
        qs = published_filter().filter(author=author).select_related('author', 'category', 'location')
    page_obj = _paginate(request, qs)
    context = {'author': author, 'page_obj': page_obj, 'post_list': page_obj.object_list}
    return render(request, 'blog/profile.html', context)


def post_detail(request, pk):
    """Детальная: автор видит свой пост всегда; остальные — только опубликованные."""
    base_qs = Post.objects.select_related('author', 'category', 'location')
    if request.user.is_authenticated:
        try:
            post = base_qs.get(pk=pk, author=request.user)
        except Post.DoesNotExist:
            post = get_object_or_404(published_filter().select_related('author', 'category', 'location'), pk=pk)
    else:
        post = get_object_or_404(published_filter().select_related('author', 'category', 'location'), pk=pk)
...
def comment_edit(request, post_id, comment_id):
    ...
    if form.is_valid():
        form.save()
        return redirect('blog:post_detail', pk=post_id)
    return render(request, 'blog/create.html', {'form': form, 'post': post, 'comment': comment, 'is_edit_comment': True})
...
def comment_delete(request, post_id, comment_id):
    ...
    if request.method == 'POST':
        comment.delete()
        return redirect('blog:post_detail', pk=post_id)
    return render(request, 'blog/create.html', {'post': post, 'comment': comment, 'is_delete_comment': True})
